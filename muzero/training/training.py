...
def train_network_helper(config: MuZeroConfig, replay_buffer: ReplayBuffer, epochs: int):
    try:
        ...
        optimizer_weights = config.old_optimizer_weight_vals('checkpoint')
        batch = replay_buffer.sample_batch(config.num_unroll_steps, config.td_steps)
        print('Setting optimizer weights')
        global bootleg_network
        bootleg_network = network
        global bootleg_batch
        bootleg_batch = replay_buffer.sample_batch(config.num_unroll_steps, config.td_steps)
        optimizer.minimize(loss=bootleg_loss, var_list=network.cb_get_variables())
        optimizer.set_weights(optimizer_weights)
    except FileNotFoundError:
        print('No checkpoint. Loading blank')
        network = config.old_network('blank_network')
        optimizer = config.new_optimizer()
        #optimizer_weights = config.old_optimizer('blank_network')
...
def bootleg_loss():
    def scale_gradient(tensor, scale: float):
        ...
        return (1. - scale) * stop_gradient(tensor) + scale * tensor
    loss = 0
    image_batch, targets_init_batch, targets_time_batch, actions_time_batch, mask_time_batch, dynamic_mask_time_batch = bootleg_batch

    # Initial step, from the real observation: representation + prediction networks
    representation_batch, value_batch, policy_batch = bootleg_network.initial_model(np.array(image_batch))
    ...
    # Compute the loss of the first pass
    loss += reduce_mean(loss_value(target_value_batch, value_batch, bootleg_network.value_support_size))
    loss += reduce_mean(
        softmax_cross_entropy_with_logits(logits=policy_batch, labels=target_policy_batch))
    ...
    for actions_batch, targets_batch, mask, dynamic_mask in zip(actions_time_batch, targets_time_batch,
        ...
        # Creating conditioned_representation: concatenate representations with actions batch
        actions_batch = one_hot(actions_batch, bootleg_network.action_size)
        ...
        # Recurrent step from conditioned representation: recurrent + prediction networks
        conditioned_representation_batch = concat((representation_batch, actions_batch), axis=3)
        representation_batch, reward_batch, value_batch, policy_batch = bootleg_network.recurrent_model(
            conditioned_representation_batch)
        ...
        # Compute the partial loss
        l = (reduce_mean(loss_value(target_value_batch, value_batch, bootleg_network.value_support_size)) +
             MSE(target_reward_batch, squeeze(reward_batch)) +
             reduce_mean(
                 softmax_cross_entropy_with_logits(logits=policy_batch, labels=target_policy_batch)))
